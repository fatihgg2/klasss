        ...
        if name[0:3] == "sud":
            ...
            if await is_on_off(config.LOG):
                ...
                return await app.send_message(
                    config.LOG_GROUP_ID,
                    f"{message.from_user.mention} kontrol etmek için botu yeni başlattım <code>SUDOLIST</code>\n\n**Kullanıcı ID:** {sender_id}\n**Kullanıcı İsmi:** {sender_name}",
                )
            return
        if name[0:3] == "lyr":
...
⚡️ __Searched Powered By {config.MUSIC_BOT_NAME}__"""
            ...
            if await is_on_off(config.LOG):
                ...
                return await app.send_message(
                    config.LOG_GROUP_ID,
                    f"{message.from_user.mention} Bota Start Verdi <code>VIDEO INFORMATION</code>\n\n**Kullanıcı ID:** {sender_id}\n**Kullanıcı İsmi:** {sender_name}",
                )
    else:
        try:
        ...
        if await is_on_off(config.LOG):
            ...
            return await app.send_message(
                config.LOG_GROUP_ID,
                f"{message.from_user.mention} Bot'a Start Verdi.\n\n**Kullanıcı ID:** {sender_id}\n**Kullanıcı İsim:** {sender_name}",
            )
    ...
    for member in message.new_chat_members:
        try:
            ...
            if member.id == app.id:
                ...
                video_url = "https://graph.org/file/c96b9226215b09ccf6d52.mp4"  # Replace with the actual URL of the video
